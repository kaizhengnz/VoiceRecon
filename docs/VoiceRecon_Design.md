...
| 2026-08-17 | Unify AI-mode selection under one field / one flag (was in-flight as issue #21 "add listen field"; design iteration during review collapsed it into this shape). `cfg["listen"]` renamed to `cfg["prompt"]` — a single string that means transcript-only when empty, resolves to a built-in preset when it matches a `presets.BUILT_IN` name (baked filter/context/trigger), and is used as a custom streaming prompt (defaults filter=them + context=current + per_segment) otherwise. Remove `--listen`, `--from`, `--context` — CLI shrinks to just `--prompt VALUE`, which follows the same disambiguation rule and overrides `cfg["prompt"]` for one session. New helper `presets.resolve(value)` centralises the disambiguation. Wizard step 6 reorders to put `meeting_summary` first and adds a "type your own prompt" entry that triggers a text follow-up. `--prompt` becomes `nargs="?"`: bare `voicerecon --prompt` runs a mini-wizard (`config.run_set_prompt`) that reuses the wizard's prompt step to change only `cfg["prompt"]`. |
