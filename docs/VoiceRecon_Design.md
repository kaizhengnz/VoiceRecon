...
| 2026-08-17 | Add a `listen` config field and a matching wizard step so a user who always runs the same preset does not have to type `--listen NAME` every launch. The wizard picks from `presets.BUILT_IN` via `_ask_choice`, including a `(none)` entry that maps to the empty string (transcript-only). CLI precedence: `--listen X` > `--prompt "..."` > `cfg["listen"]` > transcript-only. `validate_config` rejects a `listen` value that is not empty and not in `presets.BUILT_IN`, so a hand-edited typo fails at load rather than at run-start. |
